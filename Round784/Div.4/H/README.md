...
### Problem
Let AND
 denote the bitwise AND operation, and OR
...
- Select an index i
 ...
 and 30
 inclusive. In other words, in an operation you can choose an index i
 (1≤i≤n
) and set the j
-th bit of ai
 to 1
...
Output the maximum possible value of a1
 AND
 a2
 AND
 …
...
### Output
For each test case, output a single line containing the maximum possible AND
 value of a1
 AND
 a2
 AND
...
For the second test case, we can't perform any operations so the answer is just the AND
